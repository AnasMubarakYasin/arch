...
export declare type PathAble = string | RegExp | MatcherCallback;
...
export class Router implements RouterInterface {
  ...
  static register(path: PathAble, source: Router) {
    ...
    while (chunk) {
      ...
      if (subchunk.startsWith('.')) {
        ...
        const subchunk = chunk.slice(
          0,
          nextseparator > 0 ? nextseparator : undefined
        );
  ...
  constructor(path: PathAble) {
    this.Class = Router;
    this.path = path;
  }
  public Class: typeof Router;
  public path: PathAble;
  public listen(): void {
...
export class Package<T> {
  ...
  constructor(path: PathAble, data: T) {
    this.path = path;
    this.raw = data;
  }
